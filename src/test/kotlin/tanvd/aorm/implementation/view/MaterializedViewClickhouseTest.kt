...
class MaterializedViewClickhouseTest : AormTestBase() {
    ...
    fun createView_viewExists() {
        withDatabase(database) {
            ...
            Assertions.assertTrue(ExampleMaterializedView.exists())
        }
    }
    ...
    fun dropView_viewDoesNotExist() {
        withDatabase(database) {
            ...
            Assertions.assertFalse(ExampleMaterializedView.exists())
        }
    }
    ...
    fun insert_viewExistsRowValid_rowInserted() {
        withDatabase(database) {
            ...
            Assertions.assertEquals(result.single(), prepareSelectRow(mapOf(ExampleMaterializedView.date to getDate("2000-01-01"),
